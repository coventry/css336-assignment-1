...
from cs336_basics.cosine_learning_rate import cosine_learning_rate
...
def run_swiglu(
    ...
    s = SwiGLU(d_model, d_ff, w1_weight.device, w1_weight.dtype)
    s.load_state_dict(
...
def run_multihead_self_attention(
    ...
    m.load_state_dict(
        {
            "attn_projections.weight": torch.concat(
...
def run_rmsnorm(
    ...
    """
    r = RMSNorm(d_model, eps, device=weights.device, dtype=weights.dtype)
...
def get_adamw_cls() -> Any:
    ...
    class AdamWShim(AdamW):

        def __init__(self, params, lr, weight_decay, betas, eps):
            super().__init__(params, lr, betas[0], betas[1], eps, weight_decay)
